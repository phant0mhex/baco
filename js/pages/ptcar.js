...
window.pageInit = () => {
  ...
  // Attacher l'événement ici, c'est plus propre que 'oninput'
  searchBar.addEventListener('input', debouncePtCarSearch);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      let dataQuery = supabaseClient
        ...
        .order('abbr', { ascending: true }) // Trier par abbréviation
        .range(from, to);
  ...
  window.changePage = (page) => {
    ...
    currentPage = page;
    loadData();
    // Faire remonter en haut du tableau
  ...
  const renderPagination = (totalPages, totalRows, from, to) => {
    ...
    const infoHtml = `
      <p class="text-sm text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm px-3 py-1.5">
        Résultats <span class="font-bold text-gray-900">${from + 1}</span> à <span class="font-bold text-gray-900">${toRow}</span> sur
