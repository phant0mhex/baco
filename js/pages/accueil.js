   window.pageInit = () => {
      ...
      async function loadUserGreeting() {
        ...
        const avatarElement = document.getElementById('user-greeting-avatar'); // <-- NOUVEAU
        if (!greetingElement || !avatarElement) return;
        ...
        try {
          ...
          // MODIFIÉ: Ajout de 'avatar_url' au select
          const { data, error } = await supabaseClient
            .from('profiles')
            .select('full_name, avatar_url') // <-- MODIFIÉ
            .eq('id', user.id)
            .single();
            
          if (error) throw error;

          // Logique pour le texte (inchangée)
          ...
          // NOUVEAU: Logique pour l'avatar
          if (data && data.avatar_url) {
            avatarElement.src = data.avatar_url;
            avatarElement.classList.remove('hidden'); // Rendre l'avatar visible
          }

        } catch (error) {
          console.error("Erreur chargement salut :", error.message);
          greetingElement.textContent = 'Bonjour !'; // Garder le message de secours
        }
      }
